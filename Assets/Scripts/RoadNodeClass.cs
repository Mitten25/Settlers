...
public class RoadNodeClass : MonoBehaviour {
    ...
    {
        if ((phase == GameManager.state.PLACEMENT && GameManager.instance.Players[activePlayer - 1].GetComponent<PlayerClass>().initialRoad == 0 && GameManager.instance.Players[activePlayer - 1].GetComponent<PlayerClass>().initialSettlement <= 1) ||
            (phase == GameManager.state.PLACEMENT && GameManager.instance.Players[activePlayer - 1].GetComponent<PlayerClass>().initialRoad == 1 && GameManager.instance.Players[activePlayer - 1].GetComponent<PlayerClass>().initialSettlement >= 1))
        {
            if (!hasBuilding)
            {
